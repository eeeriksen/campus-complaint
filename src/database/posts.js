export const posts = [
  {
    postId: 1,
    username: "JohnDoe",
    subject: "Giant Pothole in the Student Parking Lot",
    content: "This pothole has been a nuisance for a whopping two weeks now. Everytime I drive through this parking lot I get back problems.",
    created: "Feb. 05, 2023",
    upvotes: ['eriksen', 'johndoe', 'janedoe', 'jimmydoe'],
    comments: [
      {
        commentId: 1,
        username: "JaneDoe",
        content: "Relatable.",
        created: "Feb. 05, 2023",
      },
      {
        commentId: 2,
        username: "JimmyDoe",
        content: "A lof of people can seriously get injured from this.",
        created: "Feb. 06, 2023",
      }
    ]
  },
  {
    postId: 2,
    username: "MonkeyDLuffy",
    subject: "Urinal in Building A is unflushable",
    content: "This urinal has been broken for the past three weeks and no one has done anything to fix it yet. In addition, it is the only urinal in that restroom.",
    created: "Jan. 01, 2024",
    upvotes: ['eriksen', 'johndoe', 'janedoe'],
    comments: [
      {
        commentId: 3,
        username: "Zoro",
        content: "It's seriously a hassle when I have to wait to take a quick pee but someone is already in the stall.",
        created: "Jan. 02, 2024",
      },
      {
        commentId: 4,
        username: "NamiSwan",
        content: "My guy friend told me about this and it seriously sucks.",
        created: "Jan. 04, 2024",
      }
    ]
  },
  {
    postId: 3,
    username: "ArinHandson",
    subject: "The temperature in the Computer Science department is out of control!",
    content: "On one side of the building it's freezing cold, and on the other side it's burning up! The teachers are sick of it too because all of their offices are ice cold, but no one is fixing it.",
    created: "Jan. 08, 2024",
    upvotes: ['eriksen', 'jimmydoe'],
    comments: [
      {
        commentId: 5,
        username: "JeremyRunner",
        content: "Every time I think I dressed properly for the weather, and then I get into class and it's the opposite",
        created: "Jan. 08, 2024",
      },
      {
        commentId: 6,
        username: "AngeliaFolie",
        content: "I know it doesn't sound like a big deal, but this is seriously the most inconvenient thing for me on campus.",
        created: "Jan. 10, 2024",
      },
      {
        commentId: 7,
        username: "JacobFullerton",
        content: "I'm too cold to focus on class",
        created: "Jan. 11, 2024",
      }
    ]
  },
  {
    postId: 4,
    username: "JamieSmalliver",
    subject: "The records department is so busy that it's impossible to meet with someone",
    content: "I've been in the records office every day this week for hours at a time and I'm no closer to getting my transfer credits fixed. I understand that they're busy, but something has to change, I'm worried I won't have my classes scheduled in time.",
    created: "Jan. 13, 2024",
    upvotes: ['johndoe', 'janedoe', 'jimmydoe'],
    comments: [
      {
        commentId: 8,
        username: "AndrewShmoo",
        content: "I've been here a couple years and it's only getting worse.",
        created: "Jan. 13, 2024",
      },
      {
        commentId: 9,
        username: "EricAndre",
        content: "If they're so busy, they need to hire more people!",
        created: "Jan. 13, 2024",
      }
    ]
  },
  {
    postId: 5,
    username: "OliviaRodriguez",
    subject: "Food options in the cafeteria",
    content: "Due to my religious and personal beliefs, I would really like more options in the cafeteria that do not have pork or beef.",
    created: "Jan. 15, 2024",
    upvotes: ['eriksen', 'johndoe', 'janedoe', 'jimmydoe'],
    comments: [
      {
        commentId: 10,
        username: "JulienWalls",
        content: "I can't eat gluten, I could really use more options too.",
        created: "Jan. 17, 2024",
      },
      {
        commentId: 11,
        username: "LaneyVilla",
        content: "YEESSSS! I'm vegan and sick of eating the same thing!!",
        created: "Jan. 20, 2024",
      }
    ]
  },
  {
    postId: 6,
    username: "WillBeaman",
    subject: "Colin is a really mean professor",
    content: "Colin is a seriously mean guy, I don't get why he hates me.",
    created: "Jan. 21, 2024",
    upvotes: ['jimmydoe'],
    comments: [
      {
        commentId: 12,
        username: "Anonymous",
        content: "This is just straight up wrong, Colin is super nice",
        created: "Jan. 21, 2024",
      },
      {
        commentId: 13,
        username: "Anonymous",
        content: "Just because he wants us to learn and work hard doesn't mean he hates you",
        created: "Jan. 21, 2024",
      },
      {
        commentId: 14,
        username: "Anonymous",
        content: "I agree with the other commenters, this is a you thing.",
        created: "Jan. 21, 2024",
      }
    ]
  },
  {
    postId: 7,
    username: "GarfieldAndrews",
    subject: "Help with FAFSA",
    content: "Hey everyone, this is probably a dumb question, but how do I find my FSA ID? The website keeps asking for it, but I don't remember making one.",
    created: "Jan. 22, 2024",
    upvotes: ['eriksen', 'johndoe'],
    comments: [
      {
        commentId: 15,
        username: "PhillipHoffman",
        content: "I had the same problem, there's a little button at the bottom of the email you can click to have them send you a new ID. You can search your inbox for 'Federal Student Aid' to find the right email.",
        created: "Jan. 21, 2024",
      },
      {
        commentId: 16,
        username: "GarfieldAndrews",
        content: "Thanks so much Phillip! That solved it, I've been trying to figure this out for so long!",
        created: "Jan. 21, 2024",
      },
      {
        commentId: 17,
        username: "MichaelJones",
        content: "Why did they make the button so small",
        created: "Jan. 22, 2024",
      },
      {
        commentId: 18,
        username: "AmyPond",
        content: "Please leave this post up, I never remember how to do this and I have the same problem every year.",
        created: "Mar. 02, 2024",
      }
    ]
  },
  {
    postId: 8,
    username: "LeslieKnope",
    subject: "More campus security by the rear parkking lot at night",
    content: "I have a late class two days of the week and there are no lights near the parking lot. I wish I could park in the front lot where the security people stay later. Is it possible to just have one of them come back here while the other stays in the front?",
    created: "Jan. 25, 2024",
    upvotes: ['eriksen', 'johndoe', 'janedoe', 'jimmydoe'],
    comments: [
      {
        commentId: 19,
        username: "GeorgeCastanza",
        content: "This is a good idea, I don't see why they couldn't do it",
        created: "Jan. 26, 2024",
      },
      {
        commentId: 20,
        username: "Anonymous",
        content: "I know one of the campus police, I asked him if someone could stay at the back lot longer and he said that they were supposed to be staying back there! Someone might be in trouble, but there should be someone there at night again soon.",
        created: "Jan. 27, 2024",
      }
    ]
  }
]